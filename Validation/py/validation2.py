...
for p in coef:
	...
	coefIn = []
	coefIn.append(p)
	coefIn.append(calErrRate(validationy,list(map(sign,np.dot(weight,np.transpose(validationx))))))
	coefIn.append(weight)
	dataVal.append(coefIn)
coef,rate,w = minError(dataVal)
print('the coef:',coef)
print('the error rate in train:',calErrRate(trainy,list(map(sign,np.dot(w,np.transpose(trainx))))))
print('the error rate in validation:',rate)
